...
- 默认优先按业务能力或 feature 组织模块；选择 layer-first 时必须说明规模、团队或技术原因
- 每个模块声明职责、非职责、owner、公开入口和允许依赖
- 业务逻辑不得直接依赖 HTTP、数据库、缓存、框架全局对象等具体基础设施；通过接口或适配器隔离
- shared 只能承载稳定、通用、无业务语义的能力；业务逻辑默认留在所属模块
- 新增功能应优先新增模块或扩展点，避免在核心模块堆叠条件分支
- 跨模块契约必须可测试、可校验，并记录兼容或废弃策略
- 模块必须有独立验证路径；跨模块链路补充集成或契约测试
- 架构决策必须记录维护成本、扩展成本、迁移成本和回滚方式
